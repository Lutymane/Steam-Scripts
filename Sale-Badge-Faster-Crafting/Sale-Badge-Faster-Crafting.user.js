...
//const saleAppIDs = ['991980', '876740'];

$J(() => {
    /*
    if (!saleAppIDs.includes(location.href.split('/')[6])) {
        return;
    }
*/

    if ($J('.gamecards_inventorylink').length == 0) {
        $J('.badge_detail_tasks_rule').next().prepend('<div class="gamecards_inventorylink"></div>');
        $J('.gamecards_inventorylink').append(btn_html);
    }
    else {
        $J('.gamecards_inventorylink').prepend(btn_html);
    }
