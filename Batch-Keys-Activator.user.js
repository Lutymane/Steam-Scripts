...
{
    $.post('https://store.steampowered.com/account/ajaxregisterkey/', { product_key : Keys[i], sessionid : g_sessionID }).done(function(Result){
        if ( Result !== null )
        {
            if ( Result.success == 1 )
            {
    ...
    }).fail(function(){RegisterFailure(0, null, Keys[i]);});
}
...
unsafeWindow.InitKeysRegistration = function(){
    ...
    {
        $('#error_display').text('You must agree to the terms of the Steam Subscriber Agreement!');
    }
    else
    {
